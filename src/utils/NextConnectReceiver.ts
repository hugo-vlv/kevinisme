...
// Events
export const respondToSslCheck = (req: NextApiRequest, res: NextApiResponse, next: NextHandler) => {
...
export const respondToUrlVerification = (req: NextApiRequest, res: NextApiResponse, next: NextHandler) => {
...
export default class NextConnectReceiver implements Receiver {
  ...
  public constructor({
    ...
    endpoints = { events: "/api/events", commands: "/api/commands", actions: "/api/actions" },
    ...
    this.router = createRouter<NextApiRequest, NextApiResponse>();
...
function buildBodyParserMiddleware(logger: Logger) {
  return async (req: NextApiRequest, res: NextApiResponse, next: NextHandler) => {
    let stringBody: string;
    // On some environments like GCP (Google Cloud Platform),
    // req.body can be pre-parsed and be passed as req.rawBody here
